...
cleanup() {
    ...
    # Wait for socket to be released to prevent race conditions on next run
    for i in {1..10}; do
...
info "Test 9: Soft deleting 'small.bin' (no --purge)..."
OUTPUT=$(run_zd delete "$DRIVE_NAME" small.bin 2>&1) || fail "Soft delete failed: $OUTPUT"
OUTPUT=$(run_zd list "$DRIVE_NAME" 2>&1)
if echo "$OUTPUT" | grep -q "small.bin"; then
    fail "small.bin still listed after soft delete."
fi
pass "File soft deleted from manifest."
...
# Run web UI in background, redirect stderr to log to capture port
...
# The daemon is already running from CLI tests, so /api/setup should just return a token
...
echo "$BODY" | jq -e '.node_id' > /dev/null || fail "Status response missing node_id"
...
# Reusing SMALL_FILE from CLI tests
...
# Kill the web server before finishing CLI tests
kill "$WEB_PID" >/dev/null 2>&1 || true
WEB_PID=""
